...
document.getElementById("startBtn").onclick = async () => {
    ...
    // เพิ่มบรรทัดนี้: ล้างข้อมูลประวัติเก่าใน Firebase ก่อนเริ่มใหม่
...
// --- 4. ฟังก์ชันเริ่มการฝึก (5 เซ็ต / 30 วินาที) ---
document.getElementById("startBtn").onclick = async () => {
    if (isRunning) return;
    
    // 1. ล้างข้อมูลเก่าใน Firebase ทันทีที่กดเริ่มใหม่ เพื่อให้ตารางว่างเปล่า
    await set(ref(db, 'history'), null); 
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        // บันทึกข้อมูลเข้า Firebase ทีละเซ็ต
        await set(ref(db, 'history/' + i), {
            set: i, 
            dorsi: dStr, 
            plantar: pStr, 
    ...
    // 3. เมื่อครบ 5 เซ็ต ให้เปลี่ยนปุ่มกลับมาเพื่อให้กด "เริ่มใหม่" ได้
    document.getElementById("currentActionText").innerText = "การฝึกเสร็จสมบูรณ์!";
    const startBtn = document.getElementById("startBtn");
    startBtn.disabled = false;
    startBtn.innerText = "เริ่มฝึกใหม่อีกครั้ง"; // ชื่อปุ่มจะเปลี่ยนที่ตรงนี้
    isRunning = false;
};

// --- 5. ดึงข้อมูลมาโชว์ในตาราง (จะทำงานอัตโนมัติเมื่อมีการล้างข้อมูลหรือเพิ่มข้อมูลใหม่) ---
onValue(ref(db, 'history'), (snapshot) => {
    const data = snapshot.val();
    const tbody = document.getElementById("exerciseTableBody");
    if (!tbody) return;
    
    tbody.innerHTML = ""; // ล้างหน้าจอทุกครั้งที่ข้อมูลใน Database เปลี่ยน
    if (data) {
